...
import com.example.emos.api.db.properties.AliReturnPay;
...
public class AmectController {
    ...
    public void recieveMessage(HttpServletRequest request, HttpServletResponse response, AliReturnPay aliReturnPay) throws Exception {
        // 通知返回的数据会封装到 AliReturnPay 类中
        // 支付成功的返回码
        if (("TRADE_SUCCESS").equals(aliReturnPay.getTrade_status())) {
            String outTradeNo = aliReturnPay.getOut_trade_no();
            HashMap param = new HashMap() {{
                put("status", 2);
                put("uuid", outTradeNo);
            }};
            ...
            log.error("订单状态修改失败");
            response.sendError(500, "支付成功响应异常");
        }
    }
